...
class DQNOptimizer(BaseOptimizer):
    """
    Deep Q-Network for Hyperparameter Optimization

    Uses Q-learning to learn optimal hyperparameter adjustment strategies.
    State includes current performance and parameter exploration status.
    Actions are discrete parameter modifications.
    """
    ...
    def __init__(
        ...
        """
        super().__init__("DQN", memory_size)
        ...
        # Create action space (discrete parameter modifications)
        ...
        self.exploration_counts = {}  # Track parameter region exploration

        # Initialize parameter ranges for normalization
        self.param_ranges = self._get_parameter_ranges()

        print(
            f"DQN: epsilon={epsilon}, {self.num_actions} actions, state_size={self.state_size}"
        )
        print(f"Actions: {[action['name'] for action in self.actions]}")
    ...
    def _create_action_space(self) -> List[Dict]:
        ...
        for param_name, param_config in self.hyperparameter_space.items():
            if param_config["type"] in ["int", "float"]:
                # For numerical parameters, create fine-grained increase/decrease actions
                actions.extend(
                    [
                        ...
                        # Large adjustments for major changes
                        {
        ...
        # Add multiple exploration actions with different strategies
        actions.extend(
            [
                {
        ...
        for param_name in sorted(self.hyperparameter_space.keys()):
            ...
            if param_config["type"] in ["int", "float"]:
                ...
                try:
                    ...
                    normalized.append(0.5)  # Default if value not found
    ...
    def _get_current_state(self, round_num: int) -> np.ndarray:
        ...
        # Current best score (normalized to [0, 1])
        if len(self.score_history) > 0:
            max_possible_score = 1.0  # Assuming scores are between 0 and 1
        ...
        max_rounds_without_improvement = 20  # Reasonable upper bound
        rounds_since_norm = (
        ...
        max_rounds = 100  # Reasonable estimate
        ...
        if len(self.score_history) >= 3:
            ...
            trend = (recent_scores[-1] - recent_scores[0]) / 2  # Normalized trend
            state.append(np.clip(trend, -1, 1))
        else:
            state.append(0.0)

        # Exploration density per parameter (how well each region is explored)
        for param_name in sorted(self.hyperparameter_space.keys()):
            exploration_count = self.exploration_counts.get(param_name, 0)
            max_exploration = max(10, round_num)  # Reasonable upper bound
            exploration_density = (
    ...
    def _explore_around_best(self) -> Dict:
        ...
        for param_name in params_to_modify:
            ...
            if param_config["type"] in ["int", "float"]:
                ...
                # Randomly switch to different categorical value
                if random.random() < 0.3:  # 30% chance to change categorical
    ...
    def suggest_hyperparameters(self, round_num: int) -> Dict:
        """Suggest next hyperparameters using DQN policy"""
        ...
        print(f"Round {round_num}: {decision_type} (epsilon={self.epsilon:.3f})")
        print(f"Action: {selected_action['name']}")
        print(f"Params: {new_params}")

        return new_params

    def update(self, hyperparameters: Dict, score: float):
        """Update DQN with the result of the last suggestion"""
        ...
        if len(self.score_history) == 1:
            ...
            if score > previous_best:
                ...
                reward = improvement_magnitude * 10  # Scale up improvement reward
                self.rounds_since_improvement = 0
                improvement = True
            else:
                # No improvement
        ...
        if hasattr(self, "last_state") and hasattr(self, "last_action_idx"):
            ...
            experience = (
                ...
                False,  # done (never done in our case)
            )
            self.memory.append(experience)

            # Train Q-network if we have enough experiences
    ...
    def _train_q_network(self):
        ...
        if len(self.history) % 10 == 0:  # Print every 10 updates
            print(f"  Q-network training loss: {avg_loss:.6f}")

    def get_q_values_for_debug(self, round_num: int) -> Dict:
        """Get Q-values for current state (for debugging)"""
        if self.current_params is None:
            return {}
